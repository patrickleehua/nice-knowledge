...
describe("CanonicalEntitiesPanel governance state", () => {
  ...
  test("distinguishes supported, unsupported, pinned, and merged registry entities", async () => {
    renderPanel([
      entity("entity-supported", "共享酒店实体"),
      entity("entity-unsupported", "失去来源的酒店", {
      ...
      entity("entity-pinned", "人工维护的酒店", {
        is_pinned: true,
        pinned_at: "2026-07-28T10:00:00Z",
        pinned_by: "user-1",
        pin_reason: "仍用于人工规划",
      }),
      entity("entity-merged", "旧酒店名称", {
        merged_into_entity_id: "entity-supported",
        merged_at: "2026-07-28T10:00:00Z",
        merged_by: "user-1",
        merge_reason: "与共享酒店实体重复",
      }),
    ]);

    expect(await screen.findByText("共享酒店实体")).toBeDefined();
    ...
    expect(
      screen.getByText(
        "已重定向至实体 entity-supported，原因：与共享酒店实体重复",
      ),
    ).toBeDefined();
    expect(
      screen.getByRole("button", { name: "编辑 旧酒店名称" }),
    ).toHaveProperty("disabled", true);
    expect(
      screen.getByRole("button", { name: "合并 旧酒店名称" }),
    ).toHaveProperty("disabled", true);
  });
});
